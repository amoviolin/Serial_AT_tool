...
class CDES {
#define DES_USE_TIME 8 //32 clock
public:
	uint8 KeyA[8];
	uint8 KeyB[8];
	uint8 KeyAIndex;
...
void des_function(uint8 Use3Des,uint8 Encrypt,uint8 KeyA[8],uint8 KeyB[8],uint8 Buf[8])
{
#define Report3 TRACE
	Report3 ("DES KeyA %2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x \n", 
		KeyA[0],KeyA[1],KeyA[2],KeyA[3],KeyA[4],KeyA[5],KeyA[6],KeyA[7]);
	Report3 ("DES KeyB %2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x \n", 
		KeyB[0],KeyB[1],KeyB[2],KeyB[3],KeyB[4],KeyB[5],KeyB[6],KeyB[7]);
	Report3 ("DES Data before %2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x \n", 
		Buf[0],Buf[1],Buf[2],Buf[3],Buf[4],Buf[5],Buf[6],Buf[7]);
	if (!Use3Des) {
		des_set_key( &des.ctx, KeyA );
		if( Encrypt ) des_encrypt( &des.ctx, Buf, Buf );
		else des_decrypt( &des.ctx, Buf, Buf );
	}else{
		des3_set_3keys( &des.ctx3, KeyA , KeyB,  KeyA );
		if( Encrypt ) des3_encrypt( &des.ctx3, Buf, Buf );
		else des3_decrypt( &des.ctx3, Buf, Buf );
	}
	Report3 ("DES Data after %2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x,%2.2x \n", 
		Buf[0],Buf[1],Buf[2],Buf[3],Buf[4],Buf[5],Buf[6],Buf[7]);
}